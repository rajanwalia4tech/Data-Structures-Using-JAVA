...
public class AdjacencyListImplementationWithDisconnectedComponents {
    ...
    public static void bfs(ArrayList<Edge> graph[], int start, boolean visited[]){
        ...
        q.add(start); // source node from where to start the BFS
    ...
    public static void main(String[] args) {
        ...
        */
        ...
        for(int i=0; i<graph.length;i++){ // To handle disconnected components
            if(visited[i] == false)
                bfs(graph, i, visited);
        }
        System.out.println();
